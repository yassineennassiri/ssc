...
int pvwattsv5_nofinancial_testfile(ssc_data_t &data)
{
	...
	char filepath[150];
	int n1 = sprintf(filepath, "%s/test/input_cases/pvsamv1_data/USA AZ Phoenix (TMY2).csv", std::getenv("SSCDIR"));

	//set the variables for the PVWatts default case
	ssc_data_set_string(data, "solar_resource_file", filepath); //file set above
