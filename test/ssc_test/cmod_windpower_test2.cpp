...
public:
	windpowerTestDeclaration(const char* testName, TestInfo* I, int nInfo, TestResult* R, int nRes) {
		test = new SimulationTestTable("windpower", I, nInfo, R, nRes);
...
char leapYear[150];
int c = sprintf(leapYear, "%s/test/input_docs/wind.srw", std::getenv("SSCDIR"));
