...
public:
	...
	void exec() throw(general_error)
	{
		...
		if (step_per_hour < 1 || step_per_hour > 60 || step_per_hour * 8760 != nrec)
			throw exec_error("pv_get_shade_loss", util::format("invalid number of global POA records (%d): must be an integer multiple of 8760", (int)nrec));
		double ts_hour = 1.0 / step_per_hour;

		ssc_number_t* diffuse_irrad = as_array("diffuse_irrad", &count);
		if (count != nrec)
			throw exec_error("pv_get_shade_loss", util::format("invalid number of diffuse records (%d): must be equal to other input array sizes (%d)", (int)count, (int)nrec));
		...
		if (count != nrec)
			throw exec_error("pv_get_shade_loss", util::format("invalid number of mocules per string records (%d): must be equal to other input array sizes (%d)", (int)count, (int)nrec));
...
//					if (s_sum > 0)
					if ((s_sum > 0) && (global_poa_irrad[irec] > 0))
					{
						...
						std::vector<double>vs = db8.get_vector(num_strings, diffuse_frac, s_max, counter, DB8::VS);
						std::vector<double>is = db8.get_vector(num_strings, diffuse_frac, s_max, counter, DB8::IS);
						...
						*/
						...
						else
						{
							...
							{
								if ((TcVs[i] >= v_mppt_low[irec]) && (TcVs[i] <= v_mppt_high[irec]))
								{
									if (p_fracs[i] > p_frac)
										p_frac = p_fracs[i];
								}
